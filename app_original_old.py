...
def upload_image():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_image_file(file.filename):
            ...
            python_command = "python"  # or "python3" or full path to Python executable

            # Process image file here or call your processing script
            os.system(f'{python_command} Image_AI_Keyworder.py')
            ...
            if os.path.exists(output_file):
                # Extract results from the results.pkl file
                with open("results.pkl", "rb") as f:
                    results = pickle.load(f)
                result = results.get(filename, "")
                result_entries = result.split(", ")
                title = result_entries[0]
                keywords = result_entries[1:]

                return render_template('results.html', title=title, keywords=', '.join(keywords))
            else:
...
def upload_video():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_video_file(file.filename):
            ...
            os.system(f'{python_command} Video_AI_Keyworder.py')

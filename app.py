...
def upload_image():
    ...
    user_folder = os.path.join(app.config['IMAGE_OUTPUT_FOLDER'], user_email)
    ...
    if request.method == 'POST':
        ...
        if file and allowed_image_file(file.filename):
            ...
            os.system(f'python Image_AI_Keyworder.py {filepath} {user_folder}')
            # Move the processed image to the user's folder
            processed_file = os.path.join(user_folder, filename)
            if os.path.exists(processed_file):
                flash('Your image has been successfully keyworded. Please download your JPG file(s). They will be deleted after download.')
...
def upload_video():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_video_file(file.filename):
            ...
            os.system(f'python3 Video_AI_Keyworder.py {filepath}')
